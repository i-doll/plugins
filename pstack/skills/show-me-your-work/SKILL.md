...
At the end of the run, before handing back, check the log told the truth. Read this run's own transcript, under `~/.claude/projects/<workspace-slug>/`. Don't glob across `~/.claude/projects/*/`; that reads unrelated private chats. Walk the log against what actually happened:
