...
Before spawning investigators, list the available MCP servers. Connected MCP tools are named `mcp__<server>__<tool>`; the ones already in context are visible in your tool list, and `ToolSearch` finds the rest by keyword. Search per category below rather than assuming what is connected.
...
- `model`: the `why investigators` pool from `${CLAUDE_PLUGIN_ROOT}/models.md` (or `~/.claude/pstack-models.md`, which wins) (default `sonnet`)
- Read-only by instruction, not by tooling. Investigators need their MCP tools, so don't hand them a restricted tool set that strips MCP access. Tell them to write nothing. That's a posture, not a sandbox.
...
- `subagent_type`: `general-purpose`
- `model`: the `why synthesizer` pool (default `fable`)
- Read-only by instruction. The synthesizer's quality check spot-verifies citations, which can require MCP access, so don't strip its tools. It still writes nothing.
