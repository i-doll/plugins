...
- `model`: the `how explorers` pool from `${CLAUDE_PLUGIN_ROOT}/models.md` (or `~/.claude/pstack-models.md`, which wins) (default `sonnet`)
...
- `subagent_type`: `general-purpose`
- `model`: the `how explainer` pool (default `fable`)
- Read-only: say so in the prompt. The subagent reads, greps and runs read-only commands, and edits nothing.
...
After the explanation is complete, spawn one architectural critic per model in the `how critics` pool (defaults `fable`, `opus`, `sonnet`), all in a single message.

For each critic:
- `subagent_type`: `general-purpose`
- `model`: one model from the `how critics` pool. These are minimum reasoning levels. The lead should escalate any model when the architecture warrants deeper analysis.
- Read-only: say so in the prompt. The subagent reads, greps and runs read-only commands, and edits nothing.
