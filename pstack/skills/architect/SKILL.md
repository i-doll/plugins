...
Open a task list with `TaskCreate`, one entry per phase, and move each to `in_progress`/`completed` with `TaskUpdate` as you go before starting. Autonomous mode without checkpoints needs the list to show phase position and keep phases from silently disappearing.
...
Use the `architect runners` pool from `${CLAUDE_PLUGIN_ROOT}/models.md` (or `~/.claude/pstack-models.md`, which wins) (defaults `fable`, `opus`, `sonnet`).
